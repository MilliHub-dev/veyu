...
import { getBrandNames } from "../data/vehicleBrands";

export const ServiceFilter = ({ onChange, onClose }) => {
  const services = ["Oil Change", "Tire Alignment/Rotation", "Body Work"];
    ...
    <Menu closeOnSelect={false} isOpen={isOpen} onClose={onClose}>
      ...
      <MenuList maxH="300px" overflowY="auto">
        <Box>
          ...
          {["Auto", "Manual", "Assisted Manual"].map((trans) => (
            <MenuItem
              onInput={addOrRemoveTrans}
              value={trans}
              selected={value.includes(trans)}
              key={trans}
              as={Checkbox}
            >
              {" "}
