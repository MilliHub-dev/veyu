  ...
  const fetchVerificationStatus = async () => {
    try {
      ...
      if (error.response?.status === 404) {
        ...
        console.log(
          "Verification endpoint not available - defaulting to not_submitted",
        );
    ...
    switch (status) {
      case "pending":
        return {
          ...
          message:
            "Your business verification is currently under review by our admin team. You will be notified once approved.",
          buttonText: null,
          icon: "⏳",
        };
      case "rejected":
        ...
        {Object.keys(categories)?.map((category, idx) => (
          <Box key={idx}>
            <SimpleGrid
              ...
              spacing={2}
            >
              <Text flex={1} colSpan={3} textTransform="capitalize">
                {category}
              </Text>
              <Flex alignItems="center" gap={2}>
                <Progress
                  size="sm"
                  value={categories[category] * 20}
                  borderRadius="lg"
                  flex={1}
                  colorScheme="blue"
                />
...
export function CalendarPicker({
  ...
  const handleDateSelect = (day) => {
    ...
    if (mode === "single") {
      ...
      const index = newSelectedDates.findIndex(
        (d) => d instanceof Date && isSameDay(d, day),
      );
  ...
  const isDateSelected = (day) => {
    if (mode === "single") {
      return (
        selectedDates[0] instanceof Date && isSameDay(selectedDates[0], day)
      );
    }
  ...
  const isDateHovered = (day) => {
    if (mode !== "range" || selectedDates.length !== 1 || !hoverDate)
      return false;

    const start = selectedDates[0];
    return (
      (day > start && day <= hoverDate) || (day < start && day >= hoverDate)
    );
  };
      ...
      >
        ...
        <Flex alignItems="center">
          <Button
            ...
            isDisabled={
              fromDate ? subMonths(month, 1) < startOfMonth(fromDate) : false
            }
            mr={2}
          >
            <ChevronLeftIcon />
          ...
          <Button
            ...
            isDisabled={
              toDate ? addMonths(month, 1) > startOfMonth(toDate) : false
            }
          >
            <ChevronRightIcon />
          </Button>
  ...
  const parseInitialTime = () => {
    ...
    if (typeof value === "string") {
      ...
      return {
        ...
        period:
          periodPart || (Number.parseInt(hoursPart, 10) >= 12 ? "PM" : "AM"),
      };
    }
...
export const ListingItemCard = ({ listing, ...props }) => {
  ...
  function nextImage(idx) {
    ...
    let images = (Array.isArray(rawImages) ? rawImages : [])
      .map((img) => {
        if (typeof img === "string") return { url: img };
        if (!img) return { url: "" };
        return { url: img.url || img.file || img.image || img.src || "" };
            ...
            {isOwner && (
              <IconButton
                ...
                onClick={(e) => {
                  ...
                  e.stopPropagation();
                  navigate(
                    `/dashboard/inventory/edit/${listing?.uuid || listing?.id}`,
                  );
                }}
  ...
  return (
    <form method="post" onSubmit={handleSearch}>
      <Flex
        ...
        _focusWithin={{
          borderColor: "primary",
          boxShadow: "0 0 0 3px rgba(243,159,72,0.25)",
        }}
        {...props}
      >
        <Icon className="icon" color="gray.500" fontSize={"18px"}>
          <RiSearch2Line />
        </Icon>
  ...
  return (
    <form method="post" onSubmit={handleSearch}>
      <Flex
        ...
        {...props}
      >
        <Icon className="icon" color="inherit" fontSize={"20px"}>
          <RiSearch2Line />{" "}
        </Icon>
        <Input
          type="search"
...
export const DatePicker = ({ defaultValue, onChange, ...props }) => {
  ...
  const changeVal = (e) => {
    ...
    if (!dateValue || isNaN(dateValue.getTime())) return; // Prevent empty/invalid values

    onChange(formatDateForInput(dateValue));
...
export function ImageCarousel({ images, video, ...props }) {
  ...
  const normalizedImages = (Array.isArray(images) ? images : [])
    .map((img) => {
      ...
      return {
        url: img.url || img.file || img.image || img.src || "",
        type: "image",
      };
    })
    .filter((img) => img.url);
  ...
  if (video) {
    const videoUrl =
      typeof video === "string" ? video : video.url || video.file || video.src;
    if (videoUrl) {
