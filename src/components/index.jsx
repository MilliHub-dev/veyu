...
export const VerificationNotice = ({ businessType, user, onVerification, ...props })=>{
  ...
  return(
    <Alert.Root my={4} status={"error"} colorScheme="yellow" rounded="lg" as={Stack} alignItems="start" placeItems="start">
      <Alert.Indicator as={MdWarning} w={30} h={30} />
      <Alert.Content>
        <Flex width="100%" alignItems="center" flexWrap="wrap" justify="space-between" gap={2}>
          <Alert.Title size="sm"> You have not completed your business verification. 
...
export const RatingCard = ({ avg_rating, ratings }) => {
  ...
  <Icon as={AiOutlineStar} color="yellow.400" w={6} h={6} />
      </HStack>
...
export const CustomerSearchBar = ({ onSearch, ...props }) => {
    ...
    return (
        ...
        <Flex rounded={'30px'} alignItems={'center'} zIndex={'100'} gap={2} justifyContent={'space-between'} pl={4} pr={0} py={0} border={'1px solid lightgrey'} {...props}>
            ...
            />
      <Menu.Root>
        <Menu.Trigger as={Select} color="inherit" minW="30px" maxW={'min-content'} onClick={() => setMenuState(!menuOpen)} type='button' rounded={'30px'} textTransform={'capitalize'}>
          <option value={target}>{target}</option>
        </Menu.Trigger>

        <Menu.Positioner>
          <Menu.Content minW={'max-content'} py={0}>
            <Menu.Item color="inherit" as={motion.button} type='button' onClick={() => setTarget('cars')} > Cars </Menu.Item>
            <Menu.Item color="inherit" as={motion.button} type='button' onClick={() => setTarget('mechanics')}  maxW={'max-content'}> Mechanics </Menu.Item>
          </Menu.Content>
        </Menu.Positioner>
      </Menu.Root>
        </Flex>
